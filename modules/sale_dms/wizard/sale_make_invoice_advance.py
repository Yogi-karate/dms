...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    # @api.multi
    # def _create_invoice(self, order,order_first_line, so_line, amount):
    ...
    #         'invoice_line_ids': [
    #             # (0, 0, {
    #             #     'name': order_first_line.name,
    #             #     'origin': order.name,
    #             #     'account_id': account_id,
    #             #     'price_unit': order_first_line.price_total,
    #             #     'quantity': 1.0,
    #             #     'discount': 0.0,
    #             #     'uom_id': order_first_line.product_id.uom_id.id,
    #             #     'product_id': order_first_line.product_id.id,
    #             #     'sale_line_ids': [(6, 0, [order_first_line.id])],
    #             #     'invoice_line_tax_ids': [(6, 0, tax_ids)],
    #             #     'analytic_tag_ids': [(6, 0, order_first_line.analytic_tag_ids.ids)],
    ...
    def create_invoices(self):
        ...
        if self.advance_payment_method == 'delivered':
            ...
            count = 0
            order_first_line = None
            for order in sale_orders:
                order_first_line = order.order_line[0]
                if self.advance_payment_method == 'percentage':
                    amount = order.amount_untaxed * self.amount / 100
                else:
                    amount = self.amount
